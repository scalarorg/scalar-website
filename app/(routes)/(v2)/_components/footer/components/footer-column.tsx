...
export default function FooterColumn({
  ...
  return (
    <div className={footerColumnVariants({ className })}>
      <label
        className={cn(
          'capitalize text-[16px] lg:text-[18px] 2xl:text-[22px] leading-6 xl:leading-[33px] font-bold text-black hover:text-primary-cyan-500 cursor-pointer',
        )}
      >
        {column.title}
      </label>
      ...
      <ul className={cn('flex flex-col gap-2')}>
        {column.links.map((item, i) => (
          <Link
            ...
            target="blank"
            className={cn(
              'capitalize text-[13px] xl:text-[17px] 2xl:text-[22px] leading-5 xl:leading-[24px] 2xl:leading-[33px] text-black hover:text-primary-cyan-800 ease-linear transition-all',
            )}
          >
            {item.name}
          </Link>
